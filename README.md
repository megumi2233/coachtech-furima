...
- **確認可能な主な機能（要件網羅）**:
  ...
  - **商品の購入体験**: Stripe APIを連携しており、カード決済やコンビニ払いを選択して購入手続きを完了できます。
    ...
    （※テスト環境におけるコンビニ払いの挙動については、[後述の注記](#決済機能の動作確認についてstripeテスト環境)をご確認ください）
...
### ✅ 自動テストの実行
アプリケーションの品質を担保するため、主要機能（ユーザー登録、ログイン、商品出品、バリデーション等）に対して 48 項目のフィーチャーテストを実装・実行済みです。

#### テストの網羅範囲
- **機能要件**: ID 1 〜 16 の全機能（ユーザー登録、ログイン、商品一覧・詳細、出品、購入、マイページ、住所変更など）の正常動作を網羅。
- **バリデーション**: 主要なフォーム（登録、出品、購入等）における必須入力チェックやデータ形式（数値、画像等）などの異常系動作についても検証済みです。
...
#### 3. 配送先住所未登録時の自動誘導（UXへの配慮）
商品購入フローにおいて、必須となる配送先住所が未登録の場合にエラー画面が表示されるのを防ぐため、コントローラー側で事前チェック（門番処理）を実装しました。

- **実装内容**: 購入ボタン押下時にユーザーのプロフィール情報を確認し、未登録の場合は即座に「プロフィール設定画面」へリダイレクトさせます。
- **効果**: 住所未登録によるシステムエラー（Null参照）を未然に防ぐとともに、ユーザーを迷わせることなく自然に住所登録へ誘導する導線を確保しました。
